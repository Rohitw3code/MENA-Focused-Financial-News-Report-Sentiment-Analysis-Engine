    ...
    >
        ...
        {article.sentiments && article.sentiments.length > 0 && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-2">
              {article.sentiments.slice(0, 3).map((sentiment, idx) => {
                const SentimentIcon = getSentimentIcon(sentiment.overall_sentiment);
                const EntityIcon = getEntityIcon(sentiment.entity_type);
                
                return (
                  <div key={idx} className="flex items-center justify-between p-3 bg-gradient-to-r from-slate-50 to-blue-50 rounded-lg border border-slate-200">
                    <div className="flex items-center space-x-3">
                      <div className="p-2 bg-white rounded-lg shadow-sm">
                        <EntityIcon className="h-4 w-4 text-slate-600" />
                      </div>
                      <div>
                        <div className="font-semibold text-slate-900 text-sm">{sentiment.entity_name}</div>
                        <div className="text-xs text-slate-500 capitalize">{sentiment.entity_type}</div>
                      </div>
                    </div>
                    <div className="flex space-x-2">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold border ${getSentimentColor(sentiment.financial_sentiment)}`}>
                        <SentimentIcon className="h-3 w-3 mr-1" />
                        F: {sentiment.financial_sentiment}
                      </span>
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold border ${getSentimentColor(sentiment.overall_sentiment)}`}>
                        <SentimentIcon className="h-3 w-3 mr-1" />
                        O: {sentiment.overall_sentiment}
                      </span>
                    </div>
                  </div>
                );
      ...
      <div className="px-6 py-4 bg-gradient-to-r from-slate-50 to-blue-50 border-t border-slate-100 flex items-center justify-between">
        ...
        <a
          ...
          className="flex items-center space-x-2 text-slate-600 hover:text-slate-800 font-medium transition-colors group"
        >
          <span>Source</span>
