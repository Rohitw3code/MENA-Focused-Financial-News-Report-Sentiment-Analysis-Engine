...
import { Search, Filter, TrendingUp, TrendingDown, Minus, Users, FileText, BarChart3, Brain, ChevronDown, Building2, Bitcoin, BookOpen, ArrowRight } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const [searchedEntity, setSearchedEntity] = useState<{name: string, type: string} | null>(null);
  ...
  const loadDashboardData = async () => {
    try {
      setLoading(true);
      const articlesData = await fetchData('/articles', { limit: 20 });
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      let finalEntityType = 'company'; // Default
      let entityData = null;
      
      if (matchedEntity) {
        finalEntityType = matchedEntity.entity_type;
        setSearchedEntity({ name: matchedEntity.entity_name, type: finalEntityType });
      } else {
        // Try to determine entity type by testing both
        try {
          const companyTest = await fetchData('/entity_articles_by_sentiment', {
            entity_name: searchTerm,
            entity_type: 'company'
          });
          if (companyTest && !companyTest.error) {
            finalEntityType = 'company';
          }
        } catch {
          try {
            const cryptoTest = await fetchData('/entity_articles_by_sentiment', {
              entity_name: searchTerm,
              entity_type: 'cryptocurrency'
            });
            if (cryptoTest && !cryptoTest.error) {
              finalEntityType = 'cryptocurrency';
            }
          } catch {
            // Keep default
          }
        }
        setSearchedEntity({ name: searchTerm, type: finalEntityType });
      }

      // Load the detailed entity data for the search results section
      try {
        const data = await fetchData('/entity_articles_by_sentiment', {
          entity_name: searchTerm,
  ...
  const dashboardStats = [
    { label: 'Top Entities', value: topEntities.length, color: 'primary', icon: Users },
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            {/* Company Card for Search Results */}
            {searchedEntity && (
              <motion.div
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
              ...
              >
                <CompanyCard 
                  entityName={searchedEntity.name} 
                  entityType={searchedEntity.type}
                />
              </motion.div>
            )}
                  ...
                  <div>
                    ...
                    {searchLoading ? (
                      <div className="text-center py-12">
                        ...
                        <p className="text-slate-600">Searching for entity data...</p>
                      </div>
                    ) : searchResults?.error ? (
                      <div className="text-center py-12">
                        ...
                        <p className="text-slate-600 mb-6">{searchResults.error}</p>
                        <button
                          onClick={clearSearch}
                      ...
                      <div className="space-y-6">
                        {Object.entries(searchResults).map(([category, articles]: [string, any]) => {
                          if (!Array.isArray(articles) || articles.length === 0) return null;
                          
                          const categoryLabels: Record<string, string> = {
                            positive_financial: '📈 Positive Financial',
                            negative_financial: '📉 Negative Financial',
                            neutral_financial: '📊 Neutral Financial',
                            positive_overall: '✅ Positive Overall',
                            negative_overall: '❌ Negative Overall',
                            neutral_overall: '⚪ Neutral Overall'
                          };
                          
                          return (
                            <div key={category} className="bg-card-gradient rounded-xl shadow-card border border-white/50 p-6">
                              <h3 className="text-lg font-bold text-slate-900 mb-4">
                                {categoryLabels[category]} ({articles.length} articles)
                              </h3>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {articles.slice(0, 4).map((article: any, index: number) => (
                                  <div key={index} className="p-4 bg-gradient-to-r from-slate-50 to-blue-50 rounded-lg border border-slate-200">
                                    <h4 className="font-semibold text-slate-900 mb-2 line-clamp-2">{article.title}</h4>
                                    <p className="text-sm text-slate-600 mb-3 line-clamp-2">{article.reasoning}</p>
                                    <a
                                      href={article.url}
                                      target="_blank"
                              ...
                              {articles.length > 4 && (
                                <p className="text-sm text-slate-500 mt-4 text-center">
                                  +{articles.length - 4} more articles in this category
                                </p>
                              )}
                            </div>
                          );
            ...
            >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
                  ...
                  <span className="font-bold text-blue-600 text-lg">{topEntities.filter(e => e.entity_type === 'company').length}</span>
                </div>
                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl border border-amber-100">
                  ...
                  <span className="font-bold text-amber-600 text-lg">{topEntities.filter(e => e.entity_type === 'cryptocurrency').length}</span>
                </div>
